...
describe('TaskNode agent sessions', () => {
  it('renders sessions and supports resume + remove', () => {
    ...
    const linkedSession = screen.getByTestId('task-node-agent-session-linked-agent-1')
    const recordSession = screen.getByTestId('task-node-agent-session-record-session-1')
    expect(linkedSession).toBeVisible()
    expect(linkedSession).toHaveTextContent('2026-02-08 15:20')
    expect(linkedSession).not.toHaveTextContent('gpt-5.2-codex')
    expect(linkedSession.querySelector('.agent-provider-icon')).not.toBeNull()
    expect(linkedSession.querySelector('.task-node__agent-session-state--working')).not.toBeNull()
    expect(recordSession).toBeVisible()
    expect(recordSession).toHaveTextContent('2026-02-08 15:00')
    expect(recordSession).not.toHaveTextContent('gpt-5.2-codex')
    expect(recordSession.querySelector('.agent-provider-icon')).not.toBeNull()
    expect(recordSession.querySelector('.task-node__agent-session-state--standby')).not.toBeNull()

    fireEvent.contextMenu(recordSession)
