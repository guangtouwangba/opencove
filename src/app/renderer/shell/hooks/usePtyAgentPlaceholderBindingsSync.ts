...
import type { SyncPtyAgentPlaceholderBindingsInput } from '@shared/contracts/dto'
...
function resolveBindings(workspaces: WorkspaceState[]): SyncPtyAgentPlaceholderBindingsInput {
  const bindings: SyncPtyAgentPlaceholderBindingsInput['bindings'] = []
...
export function usePtyAgentPlaceholderBindingsSync(): void {
  ...
  useEffect(() => {
    ...
    const syncSessionBindings = () => {
      const syncFn = window.opencoveApi?.pty?.syncAgentPlaceholderBindings
